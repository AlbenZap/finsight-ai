...
def plot_revenue(ticker: str, c, xbrs) -> str | None:
    ...
    try:
        ...
        if rev_rows.empty:
            rev_rows = income_df[
                income_df.concept.str.contains("Revenues|Revenue", case=False, na=False)
            ]
        if rev_rows.empty:
        ...
        net_income = (
            net_rows[income_statement.periods].iloc[0]
            if not net_rows.empty
            else pd.Series([0] * len(income_statement.periods), index=income_statement.periods)
        )
        ...
        if has_gross_profit:
            ax1.bar(
                x - width, plot_data["Revenue"], width, label="Revenue", color="#3498db", alpha=0.8
            )
            ax1.bar(
                x,
                plot_data["Gross Profit"],
                width,
                label="Gross Profit",
                color="#2ecc71",
                alpha=0.8,
        ...
        plt.title(
            f"{c.name} ({ticker}) Financial Performance", fontsize=16, fontweight="bold", pad=20
        )
        ...
        for metric in bar_metrics:
            for i in range(1, len(periods)):
                ...
                offset = (
                    list(plot_data.columns).index(metric) - len(bar_metrics) / 2 + 0.5
                ) * width
                ax1.annotate(
                    f"{growth:+.1f}%",
...
def plot_balance_sheet(ticker: str, c, xbrs) -> str | None:
    ...
    try:
        ...
        total_assets = balance_df[balance_df.concept == "us-gaap_Assets"][
            balance_sheet.periods
        ].iloc[0]
        total_liabilities = balance_df[balance_df.concept == "us-gaap_Liabilities"][
            balance_sheet.periods
        ].iloc[0]
        stockholders_equity = balance_df[balance_df.concept == "us-gaap_StockholdersEquity"][
            balance_sheet.periods
        ].iloc[0]
...
def plot_cash_flow(ticker: str, c, xbrs) -> str | None:
    ...
    try:
        ...
        operating_cf = cashflow_df[
            cashflow_df.concept == "us-gaap_NetCashProvidedByUsedInOperatingActivities"
        ][cash_flow.periods].iloc[0]
        investing_cf = cashflow_df[
            cashflow_df.concept == "us-gaap_NetCashProvidedByUsedInInvestingActivities"
        ][cash_flow.periods].iloc[0]
        financing_cf = cashflow_df[
            cashflow_df.concept == "us-gaap_NetCashProvidedByUsedInFinancingActivities"
        ][cash_flow.periods].iloc[0]
        ...
        plt.title(
            f"{c.name} ({ticker}) Cash Flow Statement", fontsize=16, fontweight="bold", pad=20
        )
        plt.figtext(0.5, 0.01, "Source: SEC EDGAR via edgartools", ha="center", fontsize=10)
